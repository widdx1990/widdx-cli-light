...
def test_uil_no_global_expert_team_import():
    ...
    source = (Path(__file__).parent / "main.py").read_text(encoding="utf-8-sig")
...
def test_main_py_uses_uil_not_expert_team():
    """The entry-point comment now mentions UIL, not expert team."""
    source = (Path(__file__).parent / "main.py").read_text(encoding="utf-8-sig")
    assert "UIL Central Brain" in source
    assert "expert team handles everything" not in source
    assert "UnifiedIntelligenceLayer.process" in source or "uil.process" in source
